...
def start_message(message):
    ...
    bot.reply_to(message, f'Привет, {user_name}, я - Серийный номер N и я люблю готовить печеньки и угощать ими других! \n' 
                                      ...
                                      'Исходники кода предоставлены мистером @Justuser_31 (Дядя Эйден)', parse_mode='HTML')
...
def help_message(message):
    bot.reply_to(message, 'aaa<blockquote>bbb</blockquote>', parse_mode='HTML')
...
def cookie_message(message):
    ...
    if (COOKIE_NUMBER >= 25):
        ...
        if (ANSWER == 11): # NEW STRING
            bot.reply_to(message, f"omagad, <tg-spoiler>SECRET COOKIE!!!!!!!!!!!!</tg-spoiler>") # NEW STRING
    if (COOKIE_NUMBER  >=1):
        if (COOKIE_NUMBER  <=24):
            if (ANSWER == 1):
...
def balance_message(message):
    ...
    if time_delta.seconds >= 1800:
        bot.reply_to(message, f'{user_name}, ты съел {user_balance} печенек\n'
                    f'Если хочешь, то можешь как раз еще поесть =3 /cookies')
